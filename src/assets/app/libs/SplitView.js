; (function () {
	...
	SplitView.activate = function (element, initialPercent = 50) {
		console.log('[SplitView.activate]', element);
		...
		if (splitView) {
			...
			// ここでinitialPercentを使用してviewAとviewBのサイズを設定
			viewA.style[dimension] = `calc(${initialPercent}% - ${gutterBounds[dimension] / 2}px)`;
			viewB.style[dimension] = `calc(${100 - initialPercent}% - ${gutterBounds[dimension] / 2}px)`;

			gutter.addEventListener("mousedown", dragStartHandler);
			gutter.addEventListener("touchstart", dragStartHandler);

			this.activate(viewA, initialPercent);  // 再帰的にactivateを呼び出す場合もinitialPercentを渡す
			this.activate(viewB, 100 - initialPercent);  // viewBの割合は100 - initialPercent
		}
	}
